...
categories: [CTF]
image: /assets/images/ais3_2025_preexam.jpg
---

**Table of Contents**
{: style="line-height: 1.2; margin-bottom: 0.5rem;"}
...
- [Pwn](#pwn)
    ...
    - [Welcome to the World of Ave Mujica🌙](#welcome-to-the-world-of-ave-mujica)
- [Misc](#misc)
    - [Ramen CTF](#ramen-ctf)
    - [AIS3 Tiny Server](#ais3-tiny-server---web--misc)
    - [Welcome](#welcome)
- [Crypto](#crypto)
    - [SlowECDSA](#slowecdsa)
    - [Stream](#stream)
    - [Hill](#hill)
    - [Random_RSA](#random_rsa)
- [Rev](#rev)
    - [Web flag checker](#web-flag-checker)
    - [AIS3 Tiny Server](#ais3-tiny-server---reverse)
{: style="line-height: 1.2; margin-top: 0.5rem;"}
...
- 進去網站可以看到 You can log in as guest/guest 嘗試用 guest/guest 登入
...
- 2FA code (後來有 burp 撈但沒截圖到)
![image](https://hackmd.io/_uploads/r17nPlAGxe.png)

看了一下 Source Code
網站使用了 SQLite 儲存使用者資料，如果沒有封鎖靜態檔案讀取，那麼這個 SQLite 檔就會像靜態檔案一樣被瀏覽器直接下載。
Source code `init.php` :
```php
...
根據題目給的圖片中發票 QRcode 掃出資訊之後至 Google maps 查詢
...
進去網站後發現有提到 root dir 下有東西嘗試塞 URL encode 來 Path Traversal
...
### Welcome
![image](https://hackmd.io/_uploads/r1pw8tafle.png)
手敲出 Flag
...
漏洞分析 漏洞點：可預測的 Nonce k
ECDSA 簽章的安全性高度依賴於每次簽章所使用的隨機數 (nonce) k 的不可預測性與唯一性。如果 k 可以被預測、重複使用或其生成方式存在缺陷，則可能導致私鑰 sk 的洩漏。
...
def sign(msg: bytes):
    ...
    # ...
```
若取得兩組簽章 (r1, s1)、(r2, s2) 對相同訊息 example_msg，就可以建立如下關係：
ECDSA 簽章公式：
$$
...
加密流程分析
`chal.py` 中的核心加密邏輯為 `hexor` 函數，其運作如下：
1. 接收兩個參數：
   - `a`: 一個位元組串（如 SHA512 雜湊或 flag）
   - `b`: 一個 256 位元隨機整數
2. 步驟：
   - 將 `a` 轉為整數：`P = int.from_bytes(a)`
   - 計算平方：`K_sq = b**2`
...
因此，有以下近似式：
C ≈ K²  ⟹  K ≈ isqrt(C)
即：只要對 `C` 取整數平方根即可近似得到 `K`！

SHA512 部分的限制
- `P_sha = int.from_bytes(sha512_hash)`，長度為 512 bits
- 此時 `P_sha` 和 `K²` 長度相當
- XOR 不再偏向低位，因此結果 `C` 和 `K²` 的差距變大
- → **平方根反推不準確，結果是亂碼**
...
3. 解密第一個明文分組：
   ```python
   P_flag[0] = (A_inv ⋅ C_flag[0]) % p
   ```
4. 解密後續明文分組：
   ```python
   for i in range(1, 5):
       term_B = (B ⋅ P_flag[i-1]) % p
       diff = (C_flag[i] - term_B + p) % p
       P_flag[i] = (A_inv ⋅ diff) % p
   ```
5. 將所有明文分組還原為字串：
   ```python
   ...
   ```
成功解出原始旗標 `flag{...}`，過程運用模矩陣運算、向量推導與線性代數反推密鑰矩陣。
...
這題 CTF (Capture The Flag) 的密碼學挑戰 "random_RSA" 要求我們破解一個 RSA 加密。其特殊之處在於 RSA 模數 `n` 的質因數 `p` 和 `q` 是由一個線性同餘產生器 (LCG) 生成的。解題的關鍵在於先還原 LCG 的參數，然後利用 `p` 和 `q` 之間已知的 LCG 生成關係來找出它們，最終解密訊息。
...
從 `h1` 和 `h2` 的定義，我們可以建立關於未知數 `a` 和 `b` （LCG 的乘數和增量）的線性方程組（模 `M`）：
...
令 `k_q` 為從 `p` 開始，呼叫 `rng` 函式直到生成質數 `q` 的總次數。則 `q` 可以表示為：
...
由於 `0 < p < M` 和 `0 < q < M`，上述同餘式意味著 `q = A_k · p + B_k - c·M` 對於某個非負整數 `c` 成立。然而，因為 `A_k · p + B_k` 的結果經過 `mod M` 運算後才是 `q`，所以通常 `c = ⌊(A_k · p + B_k) / M⌋`。

一個更為直接的方法是利用 `q ≡ A_k · p + B_k (mod M)` 和 `n = p·q`。
...
我們的策略是迭代 `k_q`（即 `genPrime(p)` 中 `rng` 的呼叫次數）。`k_q` 預期是一個相對較小的值（例如，1 到幾千）。對於每一個 `k_q` 值：
...
4. 對於得到的每個解 `x₀`，它就是 `p` 的一個候選值（因為 `0 < p < M`，所以 `p = x₀`）
   - a. 驗證 `x₀` 是否為 `n` 的因子
   - b. 若是，則計算 `q_cand = n / x₀`
   - c. 驗證 `p_cand = x₀` 和 `q_cand` 是否均為質數（可使用 `gmpy2.is_prime` 檢查）
   - d. **關鍵驗證**：模擬 `genPrime(p_cand)` 的過程，確認在恰好 `k_q` 次 `rng` 迭代後得到 `q_cand`，並且在前 `k_q - 1` 次迭代中沒有產生其他質數
...
int flagchecker(int param0) {
    ...
    loc_500001B2:
        ...
        // 必須正好是 40 bytes 才處理（5 個區塊 × 8 bytes）
        if (len == 40) {
...
![image](https://hackmd.io/_uploads/B1u-qe0zxx.png) 把題目給的檔案丟進去 IDA
`main funation` :
